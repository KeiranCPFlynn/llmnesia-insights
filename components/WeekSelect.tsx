...
export function WeekSelect({
  ...
  const optionByPeriod = new Map<string, string>();
  for (const week of weeks) {
    const period = calendarWeekStart(week);
    if (!optionByPeriod.has(period)) optionByPeriod.set(period, week);
  }
  const options = [...optionByPeriod.entries()];
  const selectedValue =
    optionByPeriod.get(calendarWeekStart(selected)) ?? selected;
  ...
  return (
    ...
    >
      {options.map(([period, week]) => (
        <option key={period} value={week}>
          Week of {formatWeek(period)}
        </option>
      ))}
    </select>
  );
