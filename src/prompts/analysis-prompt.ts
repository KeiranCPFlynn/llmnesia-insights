...
METRIC DEFINITIONS — READ BEFORE INTERPRETING. As of the July 2026 instrumentation fix, all search-engagement metrics are defined on \`search_submitted\` (one event per INTENTIONAL search), NOT the legacy \`search_performed\` (which fired on every keystroke and made searches-per-user, click rate, and zero-result rate meaningless). NEVER report, cite, or reason from any metric derived from \`search_performed\` — it is a keystroke counter retained only for historical continuity. If you see a pre-fix figure built on it, treat it as not comparable to the new series and say so rather than charting a false trend.
- WAU is now \`engagement.wau\` = distinct installs with at least one USER-INITIATED event in the week (passive lifecycle/impression/backfill traffic excluded). \`engagement.wau_any_event\` is the OLD definition (any event at all), reported alongside for one month ONLY so the discontinuity is visible — report both, and expect the new \`wau\` to read lower and to ramp up only as the fixed extension build propagates (it depends on a property that older installs don't send). Do not read the new-vs-old gap as users leaving.
- Activation (\`activation\`) now requires a \`search_submitted\` within 24h of install with timestamp at or after the install; the old ~97% figure was an artifact of an unbounded query and is not comparable.
- Retention, click rate, and zero-result rate are all measured on \`search_submitted\` / \`zero_results_submitted\` / \`result_opened\`. \`search_quality.clicks\` and \`click_rate\` exclude \`surface = popup_recents\` because an idle Recents open has no search denominator; \`clicks_including_popup_recents\` and \`click_rate_including_popup_recents\` preserve the former all-surface definition for continuity only and are not comparable to the primary series.
- Platform distribution uses \`platforms.searches\` = intentional-search result impressions from \`search_submitted.shown_{platform}\` and \`platforms.clicks\` = \`result_opened.platform_source\` from search surfaces only. \`platforms.popup_recents\` is separate because an idle Recents open has no search denominator. The frozen \`platforms.legacy\` block reproduces the old eight-slug \`search_performed.results_by_platform\` and all-surface click definitions for historical continuity only; NEVER compare it to the primary series or use it as current evidence.
- \`backfill_correlation\` tests directly whether finishing the initial backfill lifts search engagement (\`rate\` = share of installs that finished a backfill this week and searched afterward). Use it to judge the "backfill drives usage" hypothesis instead of inferring it.
